...
//        while (IfxAsclin_Asc_canWriteCount(&g_ascHandle3, 1, TIME_INFINITE) != TRUE);
//        IfxAsclin_Asc_blockingWrite(&g_ascHandle3, buffer[i]);
//    }
//}
...
//{
//    char buffer[50];
//    int integer_part = (int)num; // 정수 부분
//    int decimal_part = (int)((num - integer_part) * 1000000); // 소수 부분 (6자리)
//
//    sprintf(buffer, "%d.%06d", integer_part, decimal_part); // double 값을 문자열로 변환
//
